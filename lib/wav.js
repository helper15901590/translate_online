 ...
 * 重采样后的输出长度。
 *
 * resample() 和「这次编码会有多大」的体积估算都走这一个式子。分开各写一份的话，
 * 浮点舍入会让两者差一个采样点（比如 48k→22.05k、16000 个采样点，
 * `floor(n * to / from)` 得 7350，而 `floor(n / (from / to))` 得 7349），
 * 体积判断就会和真实长度对不上。
 */
export function resampledLength(inputLength, fromRate, toRate) {
  if (fromRate === toRate) return inputLength;
  return Math.max(0, Math.floor(inputLength / (fromRate / toRate)));
}
